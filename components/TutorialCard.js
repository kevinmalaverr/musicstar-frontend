...
export const TutorialCard = ({
  ...
  const difficultColor =
    difficult < 3 ? 'easy' : difficult === 3 ? 'medium' : 'hard';

  return (
    <article
      className="tutorial-card"
      onClick={() => history.push(tutorialUrl)}
    >
      <img
      ...
      <div className="tutorial-card__b">
        <div>
          <i className="Icon_star_outline mr-1" />
          <span>{likes}</span>
        </div>
        <div className="tutorial-card__difficult">
          <span className="mr-1">Difficult: </span>
          <div className="tutorial-card__difficult-list">
            {repeatItem(difficult, (i) => (
              <div
                key={i}
                className={`tutorial-card__difficult-bar tutorial-card__difficult-bar--${difficultColor}`}
              />
            ))}
            {repeatItem(5 - difficult, (i) => (
              <div key={i} className="tutorial-card__difficult-bar" />
            ))}
          </div>
        </div>
      </div>
