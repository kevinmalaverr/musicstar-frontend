...
export const SearchBar = () => {
  ...
  const matchedSongs = search
    ? [...lastestSearch, ...songs].filter((song) => song.match(regex))
    : lastestSearch;
  ...
  return (
    <div ref={ref} className="search-bar__container">
      <form className="search-bar" onSubmit={handleSubmit}>
        ...
        />
        <button>
          <i className="Icon_search_outline" />
        </button>
      </form>
      {focused && matchedSongs.length > 0 ? (
        <div className="search-bar-desplegable">
          {matchedSongs.map((song, i) => (
            <a className="search-bar-item" href={`song/${song}`} key={i}>
              {getFocusText(song, search, 'bold')}
            </a>
          ))}
        </div>
      ) : null}
